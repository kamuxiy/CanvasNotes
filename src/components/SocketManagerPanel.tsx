...
export function SocketManagerPanel({
  ...
  return (
    <div
      className="socket-manager-backdrop"
      data-testid="socket-manager"
      role="presentation"
      onPointerDown={(e) => {
        e.stopPropagation()
        // Ignore the opening gesture / fall-through from the context menu unmount.
        if (Date.now() - openedAtRef.current < 400) return
        if (e.target === e.currentTarget) onClose()
      }}
    >
      <div
        className="socket-manager"
        role="dialog"
        aria-modal="true"
        aria-label="节点管理"
