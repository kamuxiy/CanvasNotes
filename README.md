...
| Markdown | 支持 Markdown 正文；自动将首个一级标题 `#` 识别为组件标题；编辑 / 预览切换 |
| 开始 / 截止日期 | 日程名称 + 起止日期 |
| 信息列表 | 可增减条目的清单节点 |
| 文本框缩放 | 便签与其他文本输入框支持四向拖动调整大小 |
| 分组遮罩 | 半透明分组叠在控件上方；标题栏可拖动，组内控件可穿透点击 |
| 分组跟随 | 拖动分组时仅「开始拖动时」已在组内的控件跟随移动 |
| 自定义连接点 | 左右侧端口：通用 / 相关事件 / 日期 / 相关信息 |
| 节点管理 | 单选节点时可管理连接点名称、类型与顺序 |
| 通用互通 | **红色通用**可与任意类型连接；其他类型需同色匹配 |
| 多连线 | 同一连接点可接到多个其他连接点 |
| 方向约束 | 仅允许不同卡片的左 ↔ 右；禁止自连与同侧连线 |
| 框选 | 底部「框选」模式拖拽矩形批量选中控件 |
| 底部工具栏 | Upload Labs 风格悬浮菜单；窄窗口自动收拢为「鼠标 / 新建 / 控件操作 / 工作区」 |
| 工作区 | 新建 / 切换 / 重命名 / 导入导出本地工作区 |
| 画布右键菜单 | 空白处右键可快速新建便签、日期、清单、Markdown、分组 |
| 设置 · 关于 | 底部「设置」打开浮窗；关于页展示版本信息，并可对照 GitHub Release 检查更新 |
...
│   │   ├── GroupFrame.tsx
│   │   ├── NodeCard.tsx
│   │   ├── SettingsModal.tsx    # 设置浮窗（侧栏分类 + 关于）
│   │   ├── SocketDialog.tsx
│   │   ├── SocketManagerPanel.tsx
│   │   ├── SocketPort.tsx
│   │   ├── TitleBar.tsx
...
| 操作 | 说明 |
| :--- | :--- |
| 底部「便签 / 日期 / 清单 / Markdown / 分组」 | 在视口中心附近添加控件 |
| 画布空白右键 | 打开新建菜单 |
| 底部「设置」 | 打开设置浮窗（关于 / 检查更新） |
| 左键拖空白处 | 平移画布 |
| 滚轮 | 缩放 |
| 拖节点标题栏 | 移动节点 |
| 拖分组标题栏 | 移动分组及开始拖动时已在组内的控件 |
| 从连接点拖出 | 拉线到另一侧连接点 |
| 双击连接点 | 重命名 |
| 右键连接点 | 删除该连接点 |
| 点击连线 | 删除连线 |
